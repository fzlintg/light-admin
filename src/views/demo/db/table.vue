...
<script setup>
  ...
  import {defHttp as axios} from '@/api/http'
  ...
  const init = async () => {
      ...
      const loadData = async (db: any) => {
        const { nodeDataArray, linkDataArray } = 
          await axios.get(`/api/model/dbDiagram/${db}`)
        ...
        diagram.value.model = new go.GraphLinksModel(model.value)
      }
      onMounted(() => {
        init();
        loadData('base')
